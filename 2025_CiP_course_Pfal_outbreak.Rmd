...
```{r setup2, exec=false, eval=false}
install.packages()
```

or

```{r setup3, exec=false, eval=false}
...
BiocManager::install("dartR.base")
...
tree_plot_SEA <- tree_plot <- ggtree(tree, layout = "fan") %<+% meta_ord +
...
```{r nj-tree}
tree_all_site <- nj(dist_mat_all)

tree_all <- ggtree(tree_all_site, layout = "fan") %<+% meta_ord +
  geom_tippoint(aes(color = country), size = 2) +
  scale_color_viridis_d() +
  theme_tree2() +
  labs(title = "Neighbor-Joining Tree of *P. falciparum* Samples")
